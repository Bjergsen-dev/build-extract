...
public:
    // constructor
    Triangle() {}
    Triangle(const A_Point2d &p1, const A_Point2d &p2, const A_Point2d &p3);
    Triangle(const Triangle &triangle);

    bool computeCircumCenter();

    // judge if this triangle contains the inquired Point2d
    bool containPoint2d(const A_Point2d &p) const
    {
    ...
    bool circleContainV(const A_Point2d &v) const;
    ...
    Edge e2_;
    Edge e3_;
    A_Point2d circumCenter_;
    double radius_;
    bool isBad;
    bool isBoundary;
};

Triangle::Triangle(const A_Point2d &p1, const A_Point2d &p2, const A_Point2d &p3)
{
    p1_ = p1;
    p2_ = p2;
    p3_ = p3;
...
bool Triangle::circleContainV(const A_Point2d &v) const
{
...
bool Triangle::computeCircumCenter()
{
    ...
    A_Point2d D = (p1_ + p2_)/2;
    A_Point2d E = (p2_ + p3_)/2;
    Eigen::Vector2d a = p2_ - p1_;
    Eigen::Vector2d b = p3_ - p2_;
    Eigen::Vector2d d = A_Point2d(-a[1], a[0]);   // d is orthogonal to a
    d.normalize();
    Eigen::Vector2d e = A_Point2d(-b[1], b[0]);   // e is orthogonal to b
    e.normalize();
    Eigen::Matrix2d de;
    de << d[0], -e[0], d[1], -e[1];
