...
private:
    ...
    std::vector<A_Point2d> vertex_;
    std::vector<Edge> boundary_;
    double R_;   // the digging radius
};

DelaunayT::DelaunayT(const std::vector<A_Point2d> &points)
{
    vertex_.clear();
...
bool DelaunayT::triangulate()
{
    ...
    A_Point2d p1 = superTri_.p1_;
    A_Point2d p2 = superTri_.p2_;
    A_Point2d p3 = superTri_.p3_;
...
Triangle DelaunayT::generateSuperTri()
{
    ...
    A_Point2d p1(Xmid - 20*deltaMax, Ymid - deltaMax);
    A_Point2d p2(Xmid, Ymid + 20*deltaMax);
    A_Point2d p3(Xmid + 20*deltaMax, Ymid - deltaMax);
...
// //     int max_num_for_while = 0;
// //     while(max_num_for_while < 10)
// //     {
// //         max_num_for_while++;
//         DelaunayT Delaunay(*this);
//         Delaunay.generateAlphaShape();
