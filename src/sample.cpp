...
int main(int argc,char*argv[]) {
    ...
    //ransac(cloud, 100000, 1.5,output,cloud_other,tmp,SACMODEL_PLANE);//ransac seg the groud plane (considering ground is the biggest plane in cloud)
    filter_pcl_with_z(cloud,79,85);
    
    
    
    std::vector<PointCloud<PointXYZ>::Ptr> planars_cloud_vec = {cloud} ;
/*    std::vector<PointCloud<PointXYZ>::Ptr> planars_cloud_vec;
    region_grow (cloud,planars_cloud_vec);*///Region grow to get the groof planes and record them in planars_cloud_vec
    
    //visualization the planar pcd
    pcl::visualization::PCLVisualizer viewer("Planar Viewer");
    D3_view(viewer,planars_cloud_vec,cloud);
    
    string _name = "build_18";
    string _output = json_str_vec[4]+"/"+_name+"/";
    pcl::io::savePCDFile(_output+"planar_"+_name+".pcd",*(planars_cloud_vec[0]));
...
//   }
    ...
    float translation_x = 493213.86;//496359.75;//491644.60; // set the translation pramaters between point cloud and dom tif
    float translation_y = 5419721.72;//5419034.19;//5419357.93; // set the translation pramaters between point cloud and dom tif
...
/*        
        for(A_Point2d t_P : contours[zzc]){
            circle(edge_test_mat,Point(t_P[0],t_P[1]),1,Scalar(255,255,255),1);
        }
...
//     mat_show("copy_edge_mat",copy_edge_mat,500);
    ...
    for(A_Point2d a_point : push_to_contours2_left) {
        ...
        circle(alpha_shape_points_mat,Point(a_point[0],a_point[1]),10,Scalar(255,255,255),2);
        circle(within_lines_mat,Point(a_point[0],a_point[1]),5,Scalar(0,0,0),1);
    }
    ...
    mat_show("boundary_buffer",edge_test_mat,500);
    ...
    for(Vec4f vec : lines) {
        if(decide_line_within_pcdBoudaryArea_or_not(edge_test_mat,vec,0.5)) {
            ...
            line(within_lines_mat,Point(vec[0],vec[1]),Point(vec[2],vec[3]),Scalar(0,0,255),1,LINE_AA);
        }
    }
