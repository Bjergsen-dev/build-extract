...
bool segment::collideRay(vec2 & pt, const vec2 & src, const vec2 & drct)
{
	...
	intersection.x = ((pb.x - pa.x) * (pc.x - pd.x) * (pc.y - pa.y) -pc.x * (pb.x - pa.x) * (pc.y - pd.y) + pa.x * (pb.y - pa.y) * (pc.x - pd.x)) /((pb.y - pa.y) * (pc.x - pd.x) - (pb.x - pa.x) * (pc.y - pd.y));
	intersection.y = ((pb.y - pa.y) * (pc.y - pd.y) * (pc.x - pa.x) - pc.y * (pb.y - pa.y) * (pc.x - pd.x) + pa.y * (pb.x - pa.x) * (pc.y - pd.y))/ ((pb.x - pa.x) * (pc.y - pd.y) - (pb.y - pa.y) * (pc.x - pd.x));
